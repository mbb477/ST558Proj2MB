...
library(plotly)
# Plotly pie chart
plot_ly(cancer_data1 |> drop_na(Stage_Condensed) |> group_by(Stage_Condensed) |> 
          summarize(count = n()), labels = ~Stage_Condensed, 
        values = ~count, type = "pie") %>%
  layout(title = "Interactive Pie Chart of Cancer Stage Distriubtion in Breast Cancer",
...
b \<- ggplot(df, aes(x = wt, y = mpg)) b + geom_point(aes(color = cyl, size = qsec), alpha = 0.5) + scale_fill_manual(values = c("#00AFBB", "#E7B800", "#FC4E07")) + scale_size(range = c(0.5, 12)) \# Adjust the range of points size
...
b <- ggplot(cancer_data |>
              ...
              drop_na(MUTATION_COUNT, MSI_SCORE_MANTIS, 
                      FRACTION_GENOME_ALTERED), 
            aes(x = log2(MUTATION_COUNT + 1), y = MSI_SCORE_MANTIS))
b + geom_point(aes(color = CANCER_TYPE_DETAILED, 
                   size = FRACTION_GENOME_ALTERED), alpha = 0.4) +
  scale_fill_manual(values = c("darkturquoise", "pink", "cornflowerblue",
                                  "darkorchid", "darkseagreen", "deeppink4")) +
  labs(x = "Mutation Count", y = "Microsatellite Instability Score")
